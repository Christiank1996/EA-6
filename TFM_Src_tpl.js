var app = ( function() {
	...
	function initModels() {
        ...
        createModel("plane", "wireframe",  [0, -.5, 0], [0, 0, 0], 
            [1, 1, 1]);
		createModel("sphere", fs, [0, 0, 0], [0, 0, 0], // inner
			[.2, .2, .2]);
        createModel("sphere", fs, [1, 0, -1], [0, 0, 0], // 2
            [.2, .2, .2]);
        createModel("sphere", fs, [2, 0, 0], [0, 0, 0], // 3
            [.2, .2, .2]);
        createModel("sphere", fs, [1, 0, 1], [0, 0, 0], 
            [.2, .2, .2]);
	...
	function runMode() {
		...
		interactiveModel.rotate[2] += deltaRotate * 0.4;
		for (var i = 0; i + 2 < models.length; i++) {
			var offset = i * 36
			const angle = ((currentRotation + offset) % 144) * (2 * Math.PI / 144)
			models[i + 2].translate = [2.0 + radius * Math.cos(angle), 0, radius * Math.sin(angle)]
		}
		currentRotation += 1;
		if (currentRotation === 144) currentRotation = 0;
		render();
		setTimeout(() => {
			if (running) runMode()
		}, 25)
